...
func (s *Server) handleGetCourse() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		currentCourse, err := s.CourseService.GetCourseByID(courseID)
		if err != nil {
		...
		//remove exercises from each lesson, we just want lesson metadata
		for i := range currentCourse.Lessons {
			currentCourse.Lessons[i].Exercises = make([]course.Exercise, 0)
		}

		s.logger.Debug("Course found", "course", currentCourse.Name)
		w.Header().Set("Content-Type", "application/json")
		if err := json.NewEncoder(w).Encode(currentCourse); err != nil {
